...
#the marker is from the center and the distance away. Then it sends this information to the Arduino.  
...
while(True):
    ...
    if not ids is None:
        ...
        x_coords = [markerCorners[0][0] , markerCorners[1][0], markerCorners[2][0] ,markerCorners[3][0]]
        #centerY = markerCorners[0][1] + markerCorners[1][1] + markerCorners[2][1] + markerCorners[3][1]
        ...
        absDistance = 2
        absWidth = 109
        newDist1 = absDistance * absWidth/m_width
        newDistf = newDist1
        ...
        if reply == 2:
            sleep(.5)
            newDistf = newDistf*10
            dist = round(newDistf)
            dist = dist-4
            i2c.write_byte(ARD_ADDR,dist)
            print("this is the dist")
            print(dist)
            break
        if reply == 1:
            command = round(angle)
